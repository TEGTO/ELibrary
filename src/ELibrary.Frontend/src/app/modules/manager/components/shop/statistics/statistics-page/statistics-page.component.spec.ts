...
describe('StatisticsPageComponent', () => {
  ...
  it('should display statistics after data is fetched', () => {
    ...
    const canceledOrdersElement = statsDebugElement.query(By.css('.statistics__detail-value:nth-child(4) span')).nativeElement;
    const averagePriceElement = statsDebugElement.query(By.css('.statistics__detail-value:nth-child(5) span')).nativeElement;
    const stockAmountElement = statsDebugElement.query(By.css('.statistics__detail-value:nth-child(6) span')).nativeElement;
    const earnedMoneyElement = statsDebugElement.query(By.css('.statistics__detail-value:nth-child(7) span')).nativeElement;

    expect(inCartCopiesElement.textContent).toContain('10');
    expect(inOrderCopiesElement.textContent).toContain('15');
    expect(soldCopiesElement.textContent).toContain('100');
    expect(canceledOrdersElement.textContent).toContain('5');
