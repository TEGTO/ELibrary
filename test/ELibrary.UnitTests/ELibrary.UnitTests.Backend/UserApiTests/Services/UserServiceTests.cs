...
namespace UserApi.Services.Tests
{
    ...
    public class UserServiceTests
    {
        ...
        public async Task GetUserByUserInfoAsync_UserNotFound_ReturnsNull()
        {
            ...
            userManagerMock.Setup(x => x.FindByEmailAsync(info)).ReturnsAsync(null as User);
            userManagerMock.Setup(x => x.FindByNameAsync(info)).ReturnsAsync(null as User);
            userManagerMock.Setup(x => x.FindByIdAsync(info)).ReturnsAsync(null as User);
        ...
        public async Task SetUserRolesAsync_InvalidRole_AddToRoleFails_IdentityErrorsReturned()
        {
            ...
            userManagerMock.Setup(x => x.RemoveFromRolesAsync(
                It.IsAny<User>(),
                It.IsAny<IEnumerable<string>>())).ReturnsAsync(IdentityResult.Failed([.. identityErrors]));
        ...
        public async Task UpdateUserAsync_InvalidPasswordChange_ErrorsReturned()
        {
            ...
            userManagerMock.Setup(x => x.ChangePasswordAsync(user, updateData.OldPassword, updateData.Password))
