...
namespace UserApi.Controllers
{
    ...
    internal class UserInfoControllerTests
    {
        ...
        public async Task GetCurrentUser_ValidUser_ReturnsUserInfo()
        {
            ...
            userInfoServiceMock.Setup(x => x.GetAsync(user.Id, It.IsAny<CancellationToken>())).ReturnsAsync(userInfo);
            mapperMock.Setup(x => x.Map<UserInfoDto>(userInfo)).Returns(userInfoDto);
            // Act
            var result = await controller.GetCurrentUserInfo(CancellationToken.None);
            // Assert
